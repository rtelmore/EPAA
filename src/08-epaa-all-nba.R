...
season_sim <- 2021
results <- readRDS("data/all-pts-above-avg-30-30-5-2021.rds")
df <- results |> 
...
saveRDS(df_fig, "data/all_nba/df_fig_30_30_5.rds")
...
df <- bind_rows(readRDS("data/all_nba/df_fig_10_10_0.1.rds") |> 
...
p + geom_boxplot(stat = "identity") +
  ...
  theme(axis.text.y = element_text(face = c(rep('plain', 5), 'bold',
                                            rep('plain', 3), rep('bold', 2), 
                                            'plain', rep('bold', 2), 
                                            rep('plain', 5)),
                                   colour = c(rep('black', 5), 'maroon',
                                              rep('black', 3), rep('maroon', 2), 
                                              'black', rep('maroon', 2), 
                                              rep('black', 5)))) +
  coord_flip() +
...
p + geom_point() +
  scale_x_continuous(breaks = seq(-4, 4, by = 1), limits = c(-4, 3)) +
  scale_y_continuous(breaks = seq(-4, 15, by = 2)) +
  geom_text_repel() +
  labs(y = "Box Plus/Minus", 
       x = "Mean EPAA per Game") +
  theme_bw()
ggsave("fig/fg_pts_above_avg_vs_bpm_revision.png",
       hei = 5, wid = 6.5)
...
ggsave("fig/fg_pts_above_avg_vs_per_revision.png",
       hei = 5, wid = 6.5)
