...
## File names/paths from 03-posterior-clusters-all-players.R

tt <- readRDS(paste("data/players/posterior-clusters-",
                     dimension, "-", dimension, "-", alpha, 
                     "-2009.rds", sep = ""))

make_cluster_plots <- function(dimension = 10, alpha = 5){
  
  df <- dplyr::bind_rows(readRDS(paste("data/players/posterior-clusters-",
                                       dimension, "-", dimension, "-", alpha, 
                                       "-2010.rds", sep = "")),
                         readRDS(paste("data/players/posterior-clusters-",
                                       dimension, "-", dimension, "-", alpha, 
                                       "-2011.rds", sep = "")),
                         readRDS(paste("data/players/posterior-clusters-",
                                       dimension, "-", dimension, "-", alpha, 
  ...
  p + geom_bar(position = "stack", stat = "identity") +
    ...
    #                    dimension, ", ", alpha, ")", sep = ""))
  ggsave(paste("fig/players/posterior-shot-selection-", dimension, "-", 
...
#                       dimension, ", ", alpha, ")", sep = ""))
  ggsave(paste("fig/players/posterior-shot-making-", dimension, "-",
               dimension, "-", alpha, "-all-teams.png", sep = ""),
               hei = 12, wid = 9)
}
